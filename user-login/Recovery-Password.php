...
<?php 
	if (isset($_POST['repassbtn'])){
		$passid = $_SESSION['user_admin_info']['id'];
		$oldpassword = dataencode(dataencode($_REQUEST['oldpassword']));
		$newpassword = dataencode(dataencode($_REQUEST['newpassword']));
		$renewpassword = dataencode(dataencode($_REQUEST['renewpassword']));
		$sql1 = "SELECT * FROM user WHERE password = '$oldpassword' ";
		$result1 = mysqli_query($conn,$sql1);
		if(mysqli_num_rows($result1) > 0){
			if ($newpassword == $renewpassword) {
				$result = mysqli_query($conn,"UPDATE user SET password = '$newpassword' WHERE id = '$passid' ");
				if ($result){
